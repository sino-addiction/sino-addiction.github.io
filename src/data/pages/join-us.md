---
title: 加入我们 - SANI
description: 了解加入 SANI 的流程、合作规则与联系方式。
hero:
  kicker: Join The Network
  title: 加入我们
  subtitle: 如果您希望贡献数据、发起研究问题，或参与方法与分析合作，欢迎在明确的数据边界、署名规则与治理流程下加入 SANI 长期协作。
  primaryCta:
    label: 查看加入流程
    href: "#join-flow"
  secondaryCta:
    label: 填写数据调查表
    href: /site-survey/
  tertiaryCta:
    label: 下载合作备忘录
    href: /assets/downloads/sani-cooperation-mou-v1.doc
    download: SANI合作备忘录v1.doc
highlights:
  - label: Access
    title: 了解可共享的数据资源
    description: 在加入前明确数据类型、样本规模、采集方式与伦理状态，帮助合作快速启动。
  - label: Process
    title: 保持统一协作规则
    description: 通过数据调查表、合作备忘录和标准传输流程，明确责任分工、署名规则与数据边界。
  - label: Impact
    title: 支持多中心联合研究
    description: 中心站点统一处理后，更便于特征提取、数据库建设和跨中心比较分析。
  - label: Network
    title: 进入成员协作网络
    description: 在稳定的合作机制下共享研究计划、处理进展与联合发表机会。
steps:
  - title: 提交数据调查表
    description: 填写可共享的数据类型、样本规模、采集方式与伦理状态，帮助我们快速了解合作基础。
  - title: 确认合作并签署合作备忘录
    description: 明确数据边界、职责分工、署名规则与共享方式，完成合作备忘录签署。
  - title: 数据传输与入库
    description: 按照统一标准完成脱敏、校验和安全传输，进入中心站点。
  - title: 中心站点统一处理
    description: 进行标准化预处理、特征提取与数据库建设，并定期更新处理进展。
  - title: 开放研究提案
    description: 当数据积累到一定规模后，向成员发布可申请的研究议题，支持联合研究。
---

如果您正在建设相关队列、具备影像处理或统计建模能力，或者希望围绕特定问题发起多中心合作，欢迎与 SANI 联系。提交合作意向前，请先准备数据调查表，以便我们确认可共享数据类型、伦理状态、数据边界和传输方式。
